...
class Image {
     ...
     * @return string
     */
    static function buildImageVerify($length=4, $mode=1, $type='png', $width=60, $height=30, $verifyName='verify') {
        import('ORG.Util.String');
        $fontPath=THINK_PATH."/Library/Think/Verify/ttfs/5.ttf";
        ...
        // 干扰
        for ($i = 0; $i < 10; $i++) {
            imagearc($im, mt_rand(-10, $width), mt_rand(-10, $height), mt_rand(30, 300), mt_rand(20, 200), 78, 44, $stringColor);
        }
        for ($i = 0; $i < 25; $i++) {
        ...
        $x = $width/$length;
        for ($i = 0; $i < $length; $i++) {
            //imagestring($im,6, $i * 25 + 8, mt_rand(1, 15), $randval{$i}, $stringColor);
