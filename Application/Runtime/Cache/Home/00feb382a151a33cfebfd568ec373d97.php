	...
	<div class="span12">
		...
		<div class="portlet box green">
			...
			<div class="portlet-title">
				...
				<div class="caption"><i class="icon-reorder"></i>权限管理</div>
			...
			<div class="portlet-body form">
				...
				<!-- BEGIN FORM-->
				   ...
				   <form action="/Home/Role/editrole" method="post">
				  
				    <div class="layout_left_menu" style="width:94%;">
				    	
				    	<div class="choice">
				    	 	
				    	 	   <?php if(is_array($actions)): foreach($actions as $key=>$item): ?><fieldset id="test">
							   		
							   		 	<legend><input type="checkbox" id="all" class="all" name="act[]" <?php if(in_array(($item['id']), is_array($id['a_id'])?$id['a_id']:explode(',',$id['a_id']))): ?>checked<?php endif; ?> value="<?php echo ($item["id"]); ?>"> <?php echo ($item["action"]); ?></legend>
							    	
							    		<?php if(is_array($item["cnav"])): foreach($item["cnav"] as $key=>$i): if($i['p_id'] == $item['id']): ?><input class="simple_field label access" type="checkbox" <?php if(in_array(($i['id']), is_array($id['a_id'])?$id['a_id']:explode(',',$id['a_id']))): ?>checked<?php endif; ?>  name="act[]" value="<?php echo ($i["id"]); ?>"><span class="label"><?php echo ($i["action"]); ?></span><br/><?php endif; endforeach; endif; ?>
						  	
						  			</fieldset><br/><?php endforeach; endif; ?>
				       	 	 
				       	 	 <input type="hidden" name="role_id" value="<?php echo ($id["id"]); ?>">
				       	 	 ...
				       	 	 <script>
				       	  	 $(document).ready(function(){
				       	  	 	$("input[class=all]").css("margin-top",'-30px')
				       	  	 	$("input[class*=access]").click(function(){
				       	  	 		console.log($(this).prop('checked'));
				       	  	 		if($(this).prop('checked') == true){
				       	  	 			$(this).parent().parent().parent().children().children().children().children().prop('checked',true);
				       	  	 			$(this).parent().parent().parent().children().children().children().addClass('checked');
				       	  	 		}
