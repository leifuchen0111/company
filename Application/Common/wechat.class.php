...
class Wechat
{
	...
	const CARD_LOGO_UPLOAD_URL = '/media/uploadimg?';
	...
	///卡券相关地址
	const CARD_LOGO						  = '/media/uploadimg?';
...
<Nonce><![CDATA[%s]]></Nonce>
	 ...
	 */
	public function uploadCardImg($data,$type){
		if (!$this->access_token && !$this->checkAuth()) return false;
		//原先的上传多媒体文件接口使用 self::UPLOAD_MEDIA_URL 前缀
		if($type == 'card'){
			$result = $this->http_post(self::API_URL_PREFIX.self::CARD_LOGO_UPLOAD_URL.'access_token='.$this->access_token,$data,true);
		}else{
			$result = $this->http_post(self::UPLOAD_MEDIA_URL.self::CARD_LOGO_UPLOAD_URL.'access_token='.$this->access_token,$data,true);
		}
		if ($result)
		{
			/* $json = json_decode($result,true);
    ...
    public function addPoi($data) {
        ...
        $result = $this->http_post(self::API_URL_PREFIX . self::POI_ADD . 'access_token=' . $this->access_token,$data);
        if ($result) {
            $json = json_decode($result, true);
            /* if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
