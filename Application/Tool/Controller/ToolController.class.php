...
class ToolController extends Controller{
        ...
        if($_POST){
            ...
            if($query){
                ...
                if($User->data($data)->save()){
                    $this->success('登陆成功',I('post.callbackurl',''));
                }else{
            ...
            $this->display('Tool@Tool/login');exit;
        }
    ...
    public function showDetailHeader(){
        ...
        if(I('param.id','')){
            $len = count($rid);
            for($i=0;$i<$len;$i++){
                $ap[] = $this->ApMain->field('id,gw_id,hwserial')->getById($rid[$i]);
            }
            $id = '';
            $gw_id = '';
            ...
            $len = count($gw_id);
            for ($i=0;$i<$len;$i++){
                $ap[] = $this->ApMain->field('id,gw_id,hwserial')->getByGw_id($gw_id[$i]);
            }
            $id = '';
            $gw_id = '';
    ...
    public function treeAp($id){
        ...
        if(!$return){
            ...
            foreach ($user as $c1){
                ...
                foreach($c1 as $c2){
                    $arr[] = $this->ApMain->where(array('uid'=>$c2['id']))->select();
                }
            }
            //用户账号的直属AP
            $arr[] = $this->ApMain->where(array('uid'=>session('userId')))->select();
            $return = array();
            foreach($arr as $c1){
                foreach($c1 as $c2){
                    $return[] = $c2;
    ...
    public function treeMac($id){
        ...
        if(!$data){
            ...
            $arr = array();
            foreach($ap as $c1){
                $arr[] = $this->Mac->where(array('rid'=>$c1['id']))->select();
            }
    ...
    public function getBw($id){
        ...
        foreach($mac as $c1){
            ...
            foreach ($c1 as $c2){
                ...
                $arr['bw_up']+= $this->Bw->where(array('mid'=>$c2['id']))->sum('bw_up');
                $arr['bw_down']+= $this->Bw->where(array('mid'=>$c2['id']))->sum('bw_down');
            }
        }
    ...
    public function getFlowByAp($id){
        ...
        //获取每个AP下面的mac
        foreach($ap as $c1){
            $arr[$c1['hwserial']] = $this->Mac->where(array('rid'=>$c1['id']))->select();
        }
        ...
        foreach($arr as $k=>$v){
            ...
            foreach($v as $item){
                //  var_dump($item);
                $apBw[$apid]['bw_up']+=$this->Bw->where(array('mid'=>$item['id']))->sum('bw_up');
                $apBw[$apid]['bw_down']+=$this->Bw->where(array('mid'=>$item['id']))->sum('bw_down');
    ...
    public function getDownFlowByAp($id){
        ...
        //获取每个AP下面的mac
        foreach($ap as $c1){
            $arr[$c1['id']] = $this->Mac->where(array('rid'=>$c1['id']))->select();
        }
        // var_dump($arr);die;
        $len1 = count($arr);
        ...
        for($i=0;$i<$len1;$i++){
            $len2 = count($arr[$i]);
            for($j=0;$j<$len2;$j++){
                $apBw[$i]+= $this->Bw->where(array('mid'=>$arr[$i][$j]['id']))->sum('bw_down');
        ...
        $arr = array();
        foreach($apBw as $k=>$v){
            $arr[$k] = $this->ApMain->field('fw,rmac,id,hwserial')->getById($k);
            $arr[$k]['bw_down'] = $v;
        }
    ...
    * 用户登陆时，获取路由信息
     ...
     */
     public function fileupload(){
         ...
         $path = str_replace('.','/',urldecode(I('get.path')));
         $uploadDir = WEB_ROOT.'/'.$path;
      ...
      rename("{$filePath}_{$chunk}.parttmp", "{$filePath}_{$chunk}.part");
         ...
         if ( $done ) {
             ...
             @fclose($out);
                 ...
                 die('{"jsonrpc" : "2.0", "path" : "'.$path.'/", "filename" : "'.$fileName.'"}');
     }
     /**
     * 验证码
     */
 ...
 */
     public function show404(){
         ...
         R('Tool/Tool/showHeader');
         R('Tool/Tool/showFooter');
         $this->display('Tool@:404');
